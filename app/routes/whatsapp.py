...
async def receive_message(request: Request):
    ...
    logger.info("Message from %s (%s) type=%s", display_name, wa_number, parsed["type"])

    user_id: str | None = None
    try:
        user_id = await get_or_create_user(wa_number, display_name)
    except Exception as exc:
        logger.warning("Could not get/create user (%s) — continuing without DB", exc)

    if parsed["type"] == "interactive" and parsed["button_id"]:
        await handle_button_reply(wa_number, parsed["button_id"], parsed["button_title"], user_id=user_id)
        return PlainTextResponse(content="OK", status_code=200)
    ...
    if parsed["type"] == "audio" and parsed["audio_id"]:
        try:
            ...
            logger.info("Transcribed for %s: %r [lang=%s]", wa_number, transcript, stt_result.get("language_code"))
            if transcript:
                await handle_form(wa_number, transcript, user_id=user_id)
            else:
                await send_text(wa_number, "Sorry, I couldn't make out that voice note. Could you try again or type it?", user_id=user_id)
        except Exception as exc:
            logger.exception("Audio processing failed: %s", exc)
            await send_text(wa_number, "⚠️ Couldn't process the voice note. Please type your answer.", user_id=user_id)
        return PlainTextResponse(content="OK", status_code=200)
    ...
    await send_text(wa_number, "I can only accept text and voice notes right now. Please type your answer or send a voice note 🎙️", user_id=user_id)
    return PlainTextResponse(content="OK", status_code=200)